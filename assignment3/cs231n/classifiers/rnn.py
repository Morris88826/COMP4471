...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        out_temporal, cache_temporal = temporal_affine_forward(out_hidden, W_vocab, b_vocab)
        loss, dout = temporal_softmax_loss(out_temporal, captions_out, mask)
        
        dh, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dout, cache_temporal)
        if self.cell_type == 'rnn':
          dembed, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dh, cache_hidden)
        elif self.cell_type == 'lstm':
          dembed, dh0, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(dh, cache_hidden)
    ...
    def sample(self, features, max_length=30):
        ...
        current_word = np.array(captions[:,0], dtype=np.int32)
        prev_c = np.zeros((N, 1))
        prev_h = out_proj
        for i in range(1, max_length):
          out_embed, _ = word_embedding_forward(current_word, W_embed)
          if self.cell_type == 'rnn':
            prev_h, _ = rnn_step_forward(out_embed,prev_h, Wx, Wh, b)
          elif self.cell_type == 'lstm':
            prev_h, prev_c, _ = lstm_step_forward(out_embed, prev_h, prev_c, Wx, Wh, b)

          scores, _ = temporal_affine_forward(prev_h[:,np.newaxis,:], W_vocab, b_vocab)
          current_word = np.squeeze(np.argmax(scores, axis=2))
